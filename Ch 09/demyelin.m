...
%
% solve the demyelinated active cable problem via staggered Euler
%
% usage:  demyelin(cab,stim,pinc)
%
...
%
% e.g.:	cab = struct('rad',2e-4,'id',300,'nnor',10,'dt',.05)   prop
%      	cab = struct('rad',2e-4,'id',400,'nnor',10,'dt',.05)   block
%       stim = struct('t1',1,'t2',2,'amp',100e-6,'loc',1,'Tfin',10)
%       pinc = 5
%
%       demyelin(cab,stim,pinc)
%

function demyelin(cab,stim,pinc)
...
% now demyelinate the 6th segment 

mspat = pat;
mspat(5*(2+cab.id):5*(2+cab.id)+cab.id) = 1;
Cm = 0.01*e + (1-0.01)*mspat;
g.Cl = 0.003*e + (0.3-0.003)*mspat;
