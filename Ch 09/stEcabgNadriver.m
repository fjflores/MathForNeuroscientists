...
%
% usage:	stEcabgNadriver
%

function stEcabgNadriver

cab = struct('rad',1e-3,'ell',1,'dx',5e-3,'dt',.01);
stim = struct('t1',1,'t2',2,'amp',5e-2,'loc',5e-3,'Tfin',12);

for j=1:15
    cal(j) = 40 + (j-1)*10;
    g = struct('K', 36, 'Na', cal(j), 'Cl', 0.3);
    [r(j),spd(j)] = stEcabs2(cab,stim,g,0);
end

figure(1)
[ax,h1,h2]=plotyy(cal,spd*10,cal,r);
set(ax(1),'ycolor','k')
set(ax(2),'ycolor','r')
set(h1,'color','k')
set(h2,'color','r')
ylabel(ax(1),'Wave Speed (m/s)','fontsize',10,'color','k')
ylabel(ax(2),'V_{rest} (mV)','fontsize',10,'color','r')
box off
xlabel('g_{Na} (mS/cm^2)','fontsize',10)
hold off
...
function [r,spd] = stEcabs2(cab,stim,g,pinc)
