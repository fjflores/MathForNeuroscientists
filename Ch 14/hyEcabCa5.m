...
%
%  and gKCa  and ER with Ryanodine Receptors and IP3 Receptors
%
% usage: [ca, t] = hyEcabCa5(cab,stim,pinc)
%
%		cab.rad = cable radius (cm)
%		cab.ell = cable length (cm)
%       cab.N = number of compartments
%       cab.dt = timestep (ms)
%		stim.per = start of current pulse (ms)
%		stim.pw = end of current pulse (ms)
%       stim.Tfin = stopping time (ms)
%		stim.dose = size of current pulse (micro amps)
...
% stim = struct('t1',3,'t2',4,'Tfin',200,'Iapp',3e-4,'cstim',0)
% pinc = 0  
%
% Glu_stim example: (figs 13.16 and 17)
%
% cab = struct('rad',.0001,'ell',.1,'N',400,'dt',.02)
% stim = struct('per',50,'pw',2,'Tfin',400,'dose',1e3)
% pinc = 400
%

function [ca, t] = hyEcabCa5(cab,stim,pinc)
...
vmid = zeros(Nt+1,1); ca = vmid; bca = ca; imid = ca;
...
Diff = struct('c',220e-11,'b',110e-11,'s',220e-11,'q',110e-11,'i',200e-11);
...
M0 = [-Mrate(2) Mrate(3) 0 0;
       ...
       0 0 Mrate(7) -Mrate(8)];
M = M0;  nM = M0;
gam1 = [0 0 0 0]';  gam2 = gam1;
glu1 = 0; glu2 = glu1;
mglu1 = [0 0 0 0]'; mglu2 = mglu1;
...
eN = speye(N);
Rin = [-Diff.c*S-buff.k1*buff.BT*eN  buff.k2*eN; buff.k1*buff.BT*eN -Diff.b*S-buff.k2*eN];
Rer = [-Diff.s*S-buff.k1e*buff.QT*eN  buff.k2e*eN; buff.k1e*buff.QT*eN -Diff.q*S-buff.k2e*eN];
R = spalloc(5*N,5*N,16*N-8+3*N-2);
R(1:2*N,1:2*N) = Rin;
R(2*N+1:4*N,2*N+1:4*N) = Rer;
R(4*N+1:5*N,4*N+1:5*N) = -Diff.i*S - Mrate(8)*eN;
Rp = 2*speye(5*N) + dt*R;
Rm = 2*speye(5*N) - dt*R;
[LRm,URm] = lu(Rm);
...
y1 = yinf(cr,ir);        % IP3 gating variable
...
u = [cr; br; sr; qr; ir];           % initially at rest
...
yi = yinf(cr,ir); ty = tauy(cr,ir).*yi;
y2 = ( (2*ty-dt).*y1 + 2*yi*dt) ./ (2*ty + dt);
...
if pinc 
   ...
   hold on
   figure(3)
   plot3(x,t*ones(N,1),u(4*N+1:5*N),'color','k')
   hold on
end
...
m1KCa = m2KCa; w1 = w2; y1 = y2; mglu1 = mglu2;

for j=2:Nt,

     t = j*dt;

     glu2 = stim.dose*(mod(t,stim.per)<stim.pw);

     ip3stim = Mrate(1)*glu2;
     gam2(1) = ip3stim;

     nM(1,1) = M0(1,1) - ip3stim;
     nM(1,2) = M0(1,2) - ip3stim;

     mglu2 = (2*eye(4)/dt - nM)\ ( (2*eye(4)/dt + M)*mglu1 + gam1 + gam2 );

    
    er = Jryr(u(1:N),u(2*N+1:3*N),w1,ryr) - Jserca(u(1:N),serca) + ...
         JIP3(u(1:N),u(2*N+1:3*N),u(4*N+1:5*N),y1);

    pin = 2*cab.rad*((-ICa2+inaca)/(2*F)-Jpmca(u(1:N),pmca))/(cab.rad^2-rade^2) +...
          2*rade*er/(cab.rad^2-rade^2);
    ...
    ie0(synloc) = Mrate(7)*(mglu1(3)+mglu2(3))/2;

    q = 2*[tmp + pin; -tmp; tmpe + pe; -tmpe; ie0];
    ...
    yi = yinf(u(1:N),u(4*N+1:5*N)); ty = tauy(u(1:N),u(4*N+1:5*N)).*yi;
    y2 = ( (2*ty-dt).*y1 + 2*yi*dt) ./ (2*ty + dt);
    ...
    i1 = i2; n1 = n2; m1 = m2; h1 = h2; d1 = d2; m13h1 = m23h2; n14 = n24;
    h1N = h2N; m1N = m2N; h1T = h2T; m1T = m2T; m1L = m2L; ICa1 = ICa2;
    m1KCa = m2KCa; w1 = w2; y1 = y2; glu1 = glu2; mglu1 = mglu2; gam1 = gam2;
    M = nM;
...
subplot(4,1,4)
plot(t,imid,'linewidth',1,'color','k')
axis tight
xlabel('t  (ms)','fontsize',14)
ylabel('i (\muM)','fontsize',14)
box off
...
function val = yinf(C,I)
top = .312*(.258*C+.212e-1+.198e1*I.*C+.163*I);
val = top./(.668*I.*C+.590*I.*C.^2+.508e-1*I+.126*C+.662e-2+.557*C.^2);

function val = tauy(C,I)
top = .590*(943.+.100e4*I).*(500.*C+41);
val = top./(.668e2*I.*C+.590e2*I.*C.^2+.508e1*I+.126e2*C+.662+.557e2*C.^2);

function val = JIP3(c,s,i,y)
nuIP3 = 3e-7;
val = nuIP3*(c.*i.*y./(i+0.13)./(c+0.082)).^3.*(s-c);
